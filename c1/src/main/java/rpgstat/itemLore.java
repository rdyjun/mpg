...
public class itemLore{
    ...
    public void getNowLevelLore(Player p, String ItemName, List<String> lore){
        ...
        if(RPGSTAT.getConfig().contains(a + "stat")){  //공격력, 치명타 데미지, 이동속도, 공격속도
            for(String s : RPGSTAT.getConfig().getConfigurationSection(a + "stat").getKeys(false)){
                lore.add(ChatColor.WHITE + "  " + s + " : +" + (1 + (Integer)playerFile.getPlayerFile(p, ItemName) * Double.valueOf(String.valueOf(RPGSTAT.getConfig().get(a + "stat" + "." + s))).doubleValue()) * 100 + "%");
            }
        }
        if(RPGSTAT.getConfig().contains(a + "specific")){  //치명타확률
            for(String s : RPGSTAT.getConfig().getConfigurationSection(a + ".specific").getKeys(false)){
                lore.add(ChatColor.WHITE + "  " + s + " : +" + (Integer)playerFile.getPlayerFile(p, ItemName) / 5 * Double.valueOf(String.valueOf(RPGSTAT.getConfig().get(a + "specific" + "." + s))).doubleValue() + "%");
            }
        }
        if(RPGSTAT.getConfig().contains(a + "statper")){  //회피율, 경험치 흭득량
            for(String s : RPGSTAT.getConfig().getConfigurationSection(a + ".statper").getKeys(false)){
                lore.add(ChatColor.WHITE + "  " + s + " : +" + ((Integer)playerFile.getPlayerFile(p, ItemName) * Double.valueOf(String.valueOf(RPGSTAT.getConfig().get(a + "statper" + "." + s))).doubleValue()) + "%");
            }
        }
        if(RPGSTAT.getConfig().contains(a + "statpls")){  //체력, 기력
            for(String s : RPGSTAT.getConfig().getConfigurationSection(a + ".statpls").getKeys(false)){
                lore.add(ChatColor.WHITE + "  " + s + " : +" + ((Integer)playerFile.getPlayerFile(p, ItemName) * Double.valueOf(String.valueOf(RPGSTAT.getConfig().get(a + "statpls" + "." + s))).doubleValue()));
            }
        }
    }
    public void getNextLevelLore(Player p, String ItemName, List<String> lore){
        ...
        if(RPGSTAT.getConfig().contains(a + "stat")){  //공격력, 치명타 데미지, 이동속도, 공격속도
            for(String s : RPGSTAT.getConfig().getConfigurationSection(a + "stat").getKeys(false)){
                lore.add(ChatColor.WHITE + "  " + s + " : +" + (1 + ((Integer)playerFile.getPlayerFile(p, ItemName) + 1) * Double.valueOf(String.valueOf(RPGSTAT.getConfig().get(a + "stat" + "." + s))).doubleValue()) * 100 + "%");
            }
        }
        if(RPGSTAT.getConfig().contains(a + "specific")){  //치명타확률
            for(String s : RPGSTAT.getConfig().getConfigurationSection(a + ".specific").getKeys(false)){
                lore.add(ChatColor.WHITE + "  " + s + " : +" + ((Integer)playerFile.getPlayerFile(p, ItemName) + 1) / 5 * Double.valueOf(String.valueOf(RPGSTAT.getConfig().get(a + "specific" + "." + s))).doubleValue() + "%");
            }
        }
        if(RPGSTAT.getConfig().contains(a + "statper")){  //회피율, 경험치 흭득량
            for(String s : RPGSTAT.getConfig().getConfigurationSection(a + ".statper").getKeys(false)){
                lore.add(ChatColor.WHITE + "  " + s + " : +" + (((Integer)playerFile.getPlayerFile(p, ItemName) + 1) * Double.valueOf(String.valueOf(RPGSTAT.getConfig().get(a + "statper" + "." + s))).doubleValue()) + "%");
            }
        }
        if(RPGSTAT.getConfig().contains(a + "statpls")){  //체력, 기력
            for(String s : RPGSTAT.getConfig().getConfigurationSection(a + ".statpls").getKeys(false)){
                lore.add(ChatColor.WHITE + "  " + s + " : +" + (((Integer)playerFile.getPlayerFile(p, ItemName) + 1) * Double.valueOf(String.valueOf(RPGSTAT.getConfig().get(a + "statpls" + "." + s))).doubleValue()));
            }
        }
    }
}